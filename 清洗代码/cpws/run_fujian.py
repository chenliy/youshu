...
from IKEA.cpws.pro.fujian import fujian_article
from IKEA.cpws.pro.fujian import fujian_list
...
def process_ws(items):
    ...
    if content_type == '判决书' and trial_round == '一审':
        claim = [claim_extract(ws.claims_paragraphs) for reason in reasons if
                 reason['reason_code_level2'] == 104 or reason['reason_code_level2'] == 105][0]
        print('claim')
    else:
        claim = ''
...
if __name__ == '__main__':
    ...
    for i in range(1):
        for items in mb._execute("select * from judge_doc_fujian  limit 10".format()):
            ...
            try:
                ...
                case_no = l.get('case_no')
                if content_type == '判决书' and trial_round == '一审':
                    claim = [claim_extract(ws.claims_paragraphs) for reason in reasons if reason['reason_code_level2'] == 104 or reason['reason_code_level2'] == 105][0]
                    print('claim')
                else:
                    claim = ''
