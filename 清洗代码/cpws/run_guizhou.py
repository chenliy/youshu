...
from IKEA.cpws.pro.guizhou import guizhou_article
from IKEA.cpws.pro.guizhou import guizhou_list
...
def process_ws(items):
    l = guizhou_list(items)
    a = guizhou_article(items)
    ws = WenshuBase('\n'.join(a[3:]))
...
if __name__ == '__main__':
    ...
    for i in range(1):
        for items in mb._execute("select * from judge_doc_guizhou  limit 1000".format()):
            print(items['id'])
            try:
                l = guizhou_list(items)
                a = guizhou_article(items)
