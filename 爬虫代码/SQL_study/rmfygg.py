#!/usr/bin/env python
# -*- encoding: utf-8 -*-
# Created on 2017-12-28 13:39:32
# Project: fygg
...
class Handler(BaseHandler):
    ...
    def on_start(self):
        ...
        for i in range(len(company)):
            ...
            self.crawl(url, headers=self.headers, method='POST', data=data, callback=self.index_page,
                       save={'page': 1, 'company': company, 'data': data, 'fygg_total_num': fygg_total_num},
                       cookies=cookie, )
    ...
    def index_page(self, response):
        # 解析页面内容
        # print(response.text)
        json_list = response.json['data']  # 用于解码json数据
        company = response.save['company']
        print(response.save['data'])
        print(type(company))
        ...
        if len(json_list) == 0:
            ...
            fygg_total_num = fygg_total_num + len(json_list)
            ...
            if last_date > self.last_day:
                ...
                self.crawl(url, headers=self.headers, method='POST', data=data, callback=self.index_page,
                           save={'page': page, 'fygg_total_num': fygg_total_num}, cookies=response.cookies)
