...
class Handler(BaseHandler):
    ...
    def on_start(self):
        for i in range(1, 3):
            url = 'http://aszy.chinacourt.org/cpws/more.php?p={}&module=Paper&controller=Index&action=Index&enable=1&audit=1&foreign=0&excellent=0&cat1_id=&cat2_id=1&cat3_id=&reg_time=&casenumber=&title=&content='.format(i)
    ...
    def detail_page(self, response):
        ...
        print(court_name)
        try:
            case_no = re.compile('<!--案件号 -->(.*?)</td></tr><', re.S).findall(result)[0]
        except:
            case_no = ''
        print(case_no)
        try:
