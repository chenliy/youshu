...
class Handler(BaseHandler):
    ...
    def on_start(self):
        for i in range(1, 2):
            url = 'http://cyzy.chinacourt.org/paper/more/paper_mid/MzA0gAMA.shtml'
            self.crawl(url, headers=self.headers, save={'type1': '民事文书'}, callback=self.index_page)
        url = 'http://cyzy.chinacourt.org/paper/more/paper_mid/MzA2gAMA.shtml'
        self.crawl(url, headers=self.headers, save={'type1': '刑事文书'}, callback=self.index_page)
    @config(age=10 * 24 * 60 * 60)
    def index_page(self, response):
        type1 = response.save['type1']
        a = response.doc('div#list.font14 ul li span a[href]').items()
        b = response.doc('div#list.font14 span.right').items()
        time = []
        for each in b:
            print(each.text())
    ...
    def detail_page(self, response):
        ...
        print(court_name)
        try:
            case_no = re.compile('ass="number">(.*?)</li> ', re.S).findall(result)[0]
        except:
            case_no = ''
        print(case_no)
        try:
