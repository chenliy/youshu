...
class Handler(BaseHandler):
    ...
    def on_start(self):
        ...
        for i in range(1, 518):
            url = 'http://hnhhzy.chinacourt.org/cpws/more.php?p={}&module=Paper&controller=Index&action=Index&enable=1&audit=1&foreign=0&excellent=0&cat1_id=&cat2_id=1&cat3_id=&reg_time=&casenumber=&title=&content='.format(i)
            # print(url)
            self.crawl(url, headers=self.headers, save={'case_type': '民事文书', 'special': 0}, callback=self.index_page)
        # 刑事文书
        for k in range(1, 169):
            url = 'http://hnhhzy.chinacourt.org/cpws/more.php?p={}&module=Paper&controller=Index&action=Index&enable=1&audit=1&foreign=0&excellent=0&cat1_id=&cat2_id=2&cat3_id=&reg_time=&casenumber=&title=&content='.format(k)
            self.crawl(url, headers=self.headers, save={'case_type': '刑事文书', 'special': 0}, callback=self.index_page)

        # 行政文书
        for j in range(1, 68):
            url = 'http://hnhhzy.chinacourt.org/cpws/more.php?p={}&module=Paper&controller=Index&action=Index&enable=1&audit=1&foreign=0&excellent=0&cat1_id=&cat2_id=3&cat3_id=&reg_time=&casenumber=&title=&content='.format(j)
            self.crawl(url, headers=self.headers, save={'case_type': '行政文书', 'special': 0}, callback=self.index_page)

        # 执行文书
        for a in range(1,143):
            url = 'http://hnhhzy.chinacourt.org/cpws/more.php?p={}&module=Paper&controller=Index&action=Index&enable=1&audit=1&foreign=0&excellent=0&cat1_id=&cat2_id=4&cat3_id=&reg_time=&casenumber=&title=&content='.format(a)
