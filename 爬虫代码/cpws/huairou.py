...
class Handler(BaseHandler):
    ...
    def on_start(self):
        for i in range(1, 8):
            url = 'http://hrqfy.chinacourt.org/public/more.php?p=2&module=Paper&controller=Index&action=Index&LocationID=0800000000&enable=1&audit=1&foreign=0&excellent=0&cat1_id=&cat2_id=1.0000&cat3_id=&reg_time=&casenumber=&title=&content='.format(
                i)
            self.crawl(url, headers=self.headers, save={'type1': '民事文书'}, callback=self.index_page)
        for j in range(1, 12):
            url = 'http://hrqfy.chinacourt.org/public/more.php?p={}&module=Paper&controller=Index&action=Index&LocationID=0800000000&enable=1&audit=1&foreign=0&excellent=0&cat1_id=&cat2_id=2.0000&cat3_id=&reg_time=&casenumber=&title=&content='.format(
                j)
