...
class Handler(BaseHandler):
    ...
    def on_start(self):
        # 民事文书
        for i in range(1, 2):
            url = 'http://jdzzy.chinacourt.org/public/more.php?LocationID=0800000000&cat2_id=1.0000&foreign=0&excellent=0'
            # print(url)
            self.crawl(url, headers=self.headers, save={'case_type': '民事文书', 'special': 0}, callback=self.index_page)
        # 刑事文书
        for k in range(1, 2):
            url = 'http://jdzzy.chinacourt.org/public/more.php?LocationID=0800000000&cat2_id=2.0000&foreign=0&excellent=0'
    ...
    def detail_page(self, response):
        ...
        print(court_name)
        try:
            case_no = re.compile('--案件号 -->(.*?)</', re.S).findall(result)[0]
        except:
            case_no = ''
        print(case_no)
        try:
