...
class Handler(BaseHandler):
    ...
    def on_start(self):
        # 民事文书
        for i in range(1, 5):
            url = 'http://ljzy.chinacourt.org/paper/more/paper_mid/MzA0gAMA/page/{}.shtml'.format(i)
            # print(url)
            self.crawl(url, headers=self.headers, save={'case_type': '民事文书', 'special': 0}, callback=self.index_page)
        # 刑事文书
        for k in range(1, 176):
            url = 'http://ljzy.chinacourt.org/paper/more/paper_mid/MzAygAMA/page/{}.shtml'.format(k)
