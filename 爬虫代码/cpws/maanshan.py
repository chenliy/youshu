...
class Handler(BaseHandler):
    ...
    def on_start(self):
        # 民事文书
        for i in range(1, 2):
            url = 'http://maszy.chinacourt.org/paper/more/paper_mid/MzA0gAMA.shtml'
            # print(url)
            self.crawl(url, headers=self.headers, save={'case_type': '民事文书', 'special': 0}, callback=self.index_page)
        # 刑事文书
        for k in range(1, 2):
            url = 'http://maszy.chinacourt.org/paper/more/paper_mid/MzAygAMA.shtml'
            self.crawl(url, headers=self.headers, save={'case_type': '刑事文书', 'special': 0}, callback=self.index_page)

        # 行政文书
        for j in range(1, 2):
            url = 'http://maszy.chinacourt.org/paper/more/paper_mid/MzA2gAMA.shtml'
