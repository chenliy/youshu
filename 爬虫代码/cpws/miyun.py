...
class Handler(BaseHandler):
    ...
    def on_start(self):
        for i in range(1, 13):
            url = 'http://bjmyfy.chinacourt.org/paper/more/paper_mid/MzA0gAMA/page/{}.shtml'.format(i)
            self.crawl(url, headers=self.headers, save={'type1': '民事文书'}, callback=self.index_page)
        for j in range(1, 7):
            url = 'http://bjmyfy.chinacourt.org/paper/more/paper_mid/MzAygAMA/page/{}.shtml'.format(j)
    ...
    def detail_page(self, response):
        ...
        print(time_xq)
        try:
            content_type = re.compile('s="type">(.*?)</li>', re.S).findall(result)[0][:2] + '案件'
            case_type = re.compile('s="type">(.*?)</li>', re.S).findall(result)[0][-3:]
        except:
            content_type = ''
            case_type = ''
        print(content_type)
        ...
        print(court_name)
        try:
            case_no = re.compile('class="number">(.*?)</li', re.S).findall(result)[0]
        except:
            case_no = ''
        print(case_no)
        try:
            publish_date = re.compile(r'span>([\d-]+.*?)</spa', re.S).findall(result)[0]
        except:
            publish_date = ''
        print(publish_date)
        source = '密云区人民法院'
