...
class Handler(BaseHandler):
    ...
    def on_start(self):
        for i in range(1, 3):
            url = 'http://pjzy.chinacourt.org/public/more.php?p={}&module=Paper&controller=Index&action=Index&LocationID=0600000000&enable=1&audit=1&foreign=0&excellent=0&cat1_id=&cat2_id=1.0000&cat3_id=&reg_time=&casenumber=&title=&content='.format(
                i)
            self.crawl(url, headers=self.headers, save={'type1': '民事文书'}, callback=self.index_page)
        url = 'http://pjzy.chinacourt.org/public/more.php?LocationID=0600000000&cat2_id=2.0000&foreign=0&excellent=0'
        self.crawl(url, headers=self.headers, save={'type1': '刑事文书'}, callback=self.index_page)
        url = 'http://pjzy.chinacourt.org/public/more.php?LocationID=0600000000&cat2_id=3.0000&foreign=0&excellent=0'
        self.crawl(url, headers=self.headers, save={'type1': '行政文书'}, callback=self.index_page)

    @config(age=10 * 24 * 60 * 60)
    def index_page(self, response):
        type1 = response.save['type1']
        a = response.doc('tr td.td_line a[href]').items()
    ...
    def detail_page(self, response):
        ...
        print(case_no)
        try:
            publish_date = re.compile(r'#cc0000> ([\d-]+.*?)</FONT>', re.S).findall(result)[0]
        except:
            publish_date = ''
        print(publish_date)
        source = '盘锦法院网'
