...
class Handler(BaseHandler):
    ...
    def on_start(self):
        for i in range(1, 10):
            url = 'http://sxszzy.chinacourt.org/paper/more/paper_mid/MzAygAMA/page/{}.shtml'.format(i)
            self.crawl(url, headers=self.headers, save={'type1': '刑事文书'}, callback=self.index_page)
        for j in range(1, 3):
            url = 'http://sxszzy.chinacourt.org/paper/more/paper_mid/MzA2gAMA/page/{}.shtml'.format(j)
            self.crawl(url, headers=self.headers, save={'type1': '行政文书'}, callback=self.index_page)
        for k in range(1, 37):
            url = 'http://sxszzy.chinacourt.org/paper/more/paper_mid/MzA0gAMA/page/{}.shtml'.format(k)
            self.crawl(url, headers=self.headers, save={'type1': '民商事文书'}, callback=self.index_page)
        for z in range(1, 3):
            url = 'http://sxszzy.chinacourt.org/paper/more/paper_mid/MzAxgAMA/page/{}.shtml'.format(z)
            self.crawl(url, headers=self.headers, save={'type1': '执行文书'}, callback=self.index_page)

    @config(age=10 * 24 * 60 * 60)
    def index_page(self, response):
        type1 = response.save['type1']
        a = response.doc('div#list.font14.dian_a ul li span a[href]').items()
    ...
    def detail_page(self, response):
        ...
        print(time_xq)
        try:
            content_type = re.compile('ss="type">(.*?)</li', re.S).findall(result)[0][:2] + '案件'
            case_type = re.compile('ss="type">(.*?)</li', re.S).findall(result)[0][-3:]
        except:
            content_type = ''
            case_type = ''
        print(content_type)
