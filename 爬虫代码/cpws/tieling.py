...
class Handler(BaseHandler):
    ...
    def on_start(self):
        for i in range(1, 12):
            url = 'http://lntlzy.chinacourt.org/public/more.php?p={}&module=Paper&controller=Index&action=Index&LocationID=0600000000&enable=1&audit=1&foreign=0&excellent=0&cat1_id=&cat2_id=1.0000&cat3_id=&reg_time=&casenumber=&title=&content='.format(i)
            self.crawl(url, headers=self.headers, save={'type1': '民事文书'}, callback=self.index_page)
        for j in range(1, 2):
            url = 'http://lntlzy.chinacourt.org/public/more.php?LocationID=0600000000&cat2_id=3.0000&foreign=0&excellent=0'
            self.crawl(url, headers=self.headers, save={'type1': '行政文书'}, callback=self.index_page)
        for k in range(1, 3):
            url = 'http://lntlzy.chinacourt.org/public/more.php?p={}&module=Paper&controller=Index&action=Index&LocationID=0600000000&enable=1&audit=1&foreign=0&excellent=0&cat1_id=&cat2_id=2.0000&cat3_id=&reg_time=&casenumber=&title=&content='.format(k)
            self.crawl(url, headers=self.headers, save={'type1': '邢事文书'}, callback=self.index_page)
        url = 'http://lntlzy.chinacourt.org/public/more.php?LocationID=0600000000&cat2_id=4.0000&foreign=0&excellent=0'
        self.crawl(url, headers=self.headers, save={'type1': '执行文书'}, callback=self.index_page)
    ...
    def detail_page(self, response):
        ...
        print(content_type)
        try:
            court_name = re.compile('IZE="4">(.*?)</FON', re.S).findall(result)[0]
        except:
            court_name = ''
        print(court_name)
        try:
        ...
        print(case_no)
        try:
            publish_date = re.compile(r'c0000> ([\d-]+.*?)</FONT>', re.S).findall(result)[0]
        except:
            publish_date = ''
        print(publish_date)
        source = '铁岭市中级人民法院'
