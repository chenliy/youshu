...
class Handler(BaseHandler):
    ...
    def on_start(self):
        for i in range(1, 49):
            url = 'http://www.gdqyfy.gov.cn/qyzynet/sdgg/index_{}.jhtml'.format(i)
    ...
    def index_page(self, response):
        ...
        urls = []
        times = []
        for each in response.doc('div.fd-two01-article-li-title a[href]').items():
            urls.append(each.attr['href'])
        print(urls)
        for each in response.doc('div.fd-two01-article-li-title span').items():
    ...
    def detail_page(self, response):
        ...
        ann_content = response.doc('div.fd-three01-article-content').text()
        if not ann_content:
            ann_content = response.doc('div.fd-three01-article-content-text').text()
