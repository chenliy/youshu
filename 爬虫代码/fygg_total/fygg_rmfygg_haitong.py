...
class Handler(BaseHandler):
    ...
    crawl_config = {
        'itag': 'v7.9.1',  # 代码版本号
        'proxy': Proxy().get_common_proxy()  # 用户代理
    }
    ...
    def on_start(self):
        ...
        for i in range(len(company)):
            ...
            data = {'_noticelist_WAR_rmfynoticeListportlet_content': '',
                    '_noticelist_WAR_rmfynoticeListportlet_searchContent': '{}'.format(company[i]),
            ...
            self.crawl(url, headers=self.headers, method='POST', data=data, callback=self.update_mysql, cookies=cookie,
                       save={'company_now': company[i], 'page': '0'})
        ...
        if fygg_total_num == 0:
            ...
            for page in range(1, pages):
                url = 'https://rmfygg.court.gov.cn/web/rmfyportal/noticeinfo?p_p_id=noticelist_WAR_rmfynoticeListportlet&p_p_lifecycle=2&p_p_state=normal&p_p_mode=view&p_p_resource_id=initNoticeList&p_p_cacheability=cacheLevelPage&p_p_col_id=column-1&p_p_col_count=1#%s' % str(
                    page)
                data = {'_noticelist_WAR_rmfynoticeListportlet_content': '',
                        '_noticelist_WAR_rmfynoticeListportlet_searchContent': '{}'.format(company_now),
                ...
                self.crawl(url, headers=self.headers, method='POST', data=data, callback=self.index_page,
                           save={'page': page}, cookies=response.cookies, retries=12)
    ...
    def index_page(self, response):
        ...
        for item in json_list:
            ...
            self.crawl(url=url, headers=header, callback=self.detail_page, cookies=response.cookies, retries=12)
